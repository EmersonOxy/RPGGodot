...
Convenção obrigatória para sistemas atuais e futuros: [CONVENCOES.md](CONVENCOES.md). O Inspector deve apresentar parâmetros de design selecionados, em português e agrupados, preservando estados internos e valores existentes. Revisão estrutural: blocos 0 (convenções), 1 (Player), 2 (inimigos) e 3 (spawn) implementados. Spawners independentes com configuração e estado próprios; correção de inimigos spawnados inertes (origem capturada antes da posição final). Próximos blocos: câmera e target lock, aguardando aprovação específica.
...
Atual: Fase 3 encerrada com a aprovação do usuário. Fase 4 em andamento, itens 1 a 7 implementados. `enemy_base.gd` extrai a lógica comum; `enemy_archetype.gd` define os perfis padrão, pesado e ágil. Revisão do spawn: cada `spawn_region_3d.gd` é um spawner independente com ciclo, população, tempo, posicionamento e composição próprios; `enemy_spawn_manager.gd` foi removido e a composição migrada para `EntradaDeSpawnDeInimigo` (cena, peso relativo, ativado). Correção de inimigos spawnados inertes: `EnemyBase.setup_spawn()` define posição e origem após `add_child`, pois `_ready()` capturava `home_position` antes da posição final e o leash mantinha o inimigo em IDLE. Diagnóstico por spawner em DEBUG (somente leitura) no Inspector Remote. `difficulty_manager.gd` mantém os multiplicadores por spawner a partir de bases independentes. Testes focados retornaram `ARCHETYPES: 0 failures`, `SPAWN_REGION: 0 failures` e `DIFFICULTY: 0 failures`. Próxima entrega: objetivo simples, inimigo elite, recompensa e condição de vitória. O restante desta seção registra entregas anteriores.
